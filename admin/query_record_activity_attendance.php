...
$id  = isset($_POST['oscaID']) ? mysqli_real_escape_string($conn, trim($_POST['oscaID'])) : '';
$aid = isset($_POST['activity_id']) ? mysqli_real_escape_string($conn, trim($_POST['activity_id'])) : '';
$date = date('Y-m-d');
$time = date('H:i:s');
...
if (!$seniorCheck || mysqli_num_rows($seniorCheck) === 0) {
...
mysqli_query($conn, "INSERT INTO transaction_logs (OscaIDNo, ActivityID, DateRecorded, TimeRecorded, Status) 
                     VALUES ('$id', '$aid', '$date', '$time', 'Present')");
